    ...
    const fetchExp = async () => {
      try {
        ...
        if (found) {
          ...
          // Fallback to mock
          const mock = cvData.experience.find(exp => exp.id === id);
          if (mock) setExperience(mock);
          else navigate('/404');
        }
      } catch (error) {
        const mock = cvData.experience.find(exp => exp.id === id);
    ...
    <div className="min-h-screen bg-background">
      <section className="relative h-[70vh] w-full overflow-hidden">
        <motion.div style={{ opacity, scale }} className="absolute inset-0">
          <img 
            src={experience.coverImage || 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=2070'} 
          ...
          <div className="max-w-7xl mx-auto w-full">
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-6 uppercase leading-[0.9]">
                {experience.title.split('&').map((part, i) => (
                  <span key={i} className={i % 2 === 1 ? 'text-primary block md:inline' : 'block md:inline'}>
                    {i > 0 && ' & '}
                    {part.trim()}
                  </span>
                ))}
            ...
            <div className="lg:col-span-8 space-y-24">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">Contexte</span>
                </div>
              ...
              {experience.stack && (
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <div className="flex items-center gap-3 mb-12">
                    <div className="w-12 h-[2px] bg-primary rounded-full" />
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">Stack Technique</span>
                  </div>
              ...
              {experience.challenges && (
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <div className="flex items-center gap-3 mb-12">
                    <div className="w-12 h-[2px] bg-primary rounded-full" />
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">Défis & Solutions</span>
                  </div>
              ...
              {(experience.solutionDiagram || experience.impactGraph) && (
                <section className="space-y-12">
                  <div className="flex items-center gap-3 mb-8">
                    <div className="w-12 h-[2px] bg-primary rounded-full" />
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">Visualisation Technique</span>
                  </div>
                  <VisualDiagram solutionDiagram={experience.solutionDiagram} impactGraph={experience.impactGraph} />
                </section>
              )}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">Extrait de Code</span>
                </div>
                <div className="rounded-3xl overflow-hidden border border-border shadow-2xl">
                  <CodeHighlighter code={`// Example implementation for ${experience.company}
async function handleCoreTask() {
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="sticky top-32 space-y-12">
                <div className="p-10 rounded-[2.5rem] bg-card border border-border shadow-sm">
                  <div className="flex items-center gap-3 mb-10">
                    ...
                    <h3 className="text-xl font-black uppercase tracking-tight">Réalisations</h3>
                  </div>
                  <div className="space-y-8">
                  ...
                  <div className="p-10 rounded-[2.5rem] bg-secondary/30 border border-border">
                    ...
                    <div className="space-y-4">
                      {experience.links.map((link, idx) => (
                        <a key={idx} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 rounded-xl bg-background border border-border hover:border-primary/50 transition-all group">
